...
import { MailService } from 'src/app/Service/mail.service';
  ...
  export class ValiderDem2Component {
    mails : Email[] = [];
    mail : Email;
    ...
    private getmail(): void {
      ...
      this.demandeService.getById(idDemande).subscribe(demande => {
        this.statusOptions = Object.values(Status).filter(status =>
          status === Status.terminée || status === Status.refusée || status === Status.en_cours_1ere_validation
        );
    ...
    onEnvoyerMail() {
      // Ajoute le statut actuel de la demande au corps de l'email
      this.mailForm.patchValue({
        ...
        body: `<div style="font-family: Arial, sans-serif;">
            ...
            <p>Statut: <span style="color: ${this.demande.statut === 'terminée' ? 'green' : 'red'};">
              <i class="fa ${this.demande.statut === 'terminée' ? 'fa-check-circle' : 'fa-times-circle'}"></i>
      ...
      if (this.mailForm.valid) {
        ...
        this.emailService.addMail(this.formData).subscribe(
          data => {
            // Affiche le texte de succès renvoyé par le serveur
            Swal.fire({
              icon: 'success',
              title: 'Succès !',
              color: 'green',
